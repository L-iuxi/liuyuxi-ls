...
// -l
void printf_long(const char *file_name, const struct stat *file_stat, int ls_i, int ls_s) {
    if (ls_i) {
...
void list(const char *path, int ls_all, int ls_long, int ls_R, int reverse, int time, int ls_i, int ls_s) {
    struct dirent *entry;
    ...
    for (int i = 0; i < num_files; i++) {
        ...
        //  printf("%lu ", file_stat->st_ino);
           char file_path[1024];
        snprintf(file_path, sizeof(file_path), "%s/%s",path,file_list[i]->d_name);
